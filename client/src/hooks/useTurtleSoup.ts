...
const NOTICE_KEY = 'csgofriberg_soup_update';
...
export function useTurtleSoup(mode: string) {
  ...
  const fail = useCallback((err: unknown) => {
    setError(errMsg(err));
    if (axios.isAxiosError(err) && err.response && err.response.status < 500) syncPaused.current = true;
    if (missing(err)) {
      setExpired(true);
      setOptions(null);
      save({});
  ...
  const notify = () => {
    try { localStorage.setItem(NOTICE_KEY, crypto.randomUUID()); } catch { /* Focus and polling also sync. */ }
  };
  ...
  const sync = useCallback(async () => {
    if (inFlight.current || syncPaused.current || session.current.pending || !session.current.gameId || expired) return;
    const generation = epoch.current;
    const id = session.current.gameId;
    try {
      const res = await api.get<SoupGame>(`/game/${id}/state`);
      // A background read cannot overwrite a write or a newer server version.
      if (epoch.current !== generation || inFlight.current || session.current.pending || session.current.gameId !== id ||
          res.data.version < (currentGame.current?.version ?? 0)) return;
      accept(res.data);
      // If the options request failed, keep its retry visible until load() can
      // fetch the snapshot. A successful state poll alone does not repair it.
      if (choicesFor.current === id || res.data.status !== 'playing') setError(null);
    } catch (err) {
      if (epoch.current === generation && !inFlight.current && !session.current.pending && session.current.gameId === id) fail(err);
    }
  }, [accept, expired, fail]);

  useEffect(() => {
    const wake = () => { if (document.visibilityState === 'visible') void sync(); };
    const changed = (event: StorageEvent) => { if (event.key === NOTICE_KEY) void sync(); };
