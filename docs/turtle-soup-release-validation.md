...
- 账号/后台串联使用本轮创建的测试身份、数据库副本 `.local-data/stage3-acceptance.sqlite3`、编译后页面 `http://localhost:3002`、独立 Redis 前缀 `csgofriberg-stage3-acceptance:`。仅副本内修改测试选手昵称/年龄。
...
| 真实 30 分钟过期 | 执行中 | Edge 两标签，独立访客局，从真实 `lastActiveAt` 计时，周期读取状态和 Redis TTL，不更改 TTL；证据 `stage3-expiry.log/json` |
