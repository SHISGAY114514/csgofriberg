...
这份文档是海龟汤功能的当前维护入口，汇总运行模型、前端入口、数据迁移和历史验收。完整接口字段见[接口约定](turtle-soup-api.md)。先前分阶段交接和提示词中的环境路径及任务状态可能已经过时，当前以本文件和实际代码为准。

## 当前运行模型

- 每局最多 18 次属性提问；提问、猜名、认输均由服务端校验，成功响应返回完整局面快照。
- 游戏页面在本地维护当前快照。正常操作不轮询 `/state`，也不因焦点、可见性或跨标签事件重复请求。
- 首次进入或刷新已知局面时读取一次 state；用户点击重试、恢复未确认操作或处理需要同步的明确错误时才再次读取。
- 每个写操作保留同一个 `requestId`、`version` 和请求体到确认；网络错误或 5xx 不会创建第二个操作。
- 退出／重开清除 Redis 中的进行中状态且不写入历史；终局才结算数据库记录。迁移可重复执行。
- 个人战绩、排行榜和回放按 `variant=turtle-soup` 与普通单人游戏隔离；海龟汤提问数与猜名次数分别统计。

## 搜索行为

战队和国家／地区候选均来自本局 `question-options` 快照，输入后在浏览器本地筛选。国家／地区匹配当前语言名称、服务端原始值、中文拼音全拼和拼音首字母；地区名称也作为候选分组的搜索字段。鼠标点击或按 Enter/Tab 选中候选后立即提交提问，普通输入和输入法组字不会触发提交。已经提交过的候选再次聚焦时会自动全选当前文本，方便直接替换或删除。

## 代码入口

| 范围 | 文件 |
| --- | --- |
| 页面和本地状态 | `client/src/pages/TurtleSoupGame.tsx`、`client/src/hooks/useTurtleSoup.ts` |
| 候选输入和搜索 | `client/src/components/SoupOptionInput.tsx`、`client/src/utils/playerGeography.ts` |
| 服务端规则和路由 | `server/src/services/turtleSoup.ts`、`server/src/routes/game.ts` |
| 数据结构 | `server/src/db/schema.ts`、`server/src/db/ready.ts` |
| 行为测试 | `client/test/pages/TurtleSoupGame.test.tsx`、`client/test/utils/playerGeography.test.ts`、`server/test/routes/turtleSoup.integration.test.ts` |
...
生产环境必须先备份 PostgreSQL，再运行迁移后启动服务；本地 SQLite 迁移不会替代生产验收。不要提交本地数据库、凭据、浏览器截图或 `.local-*` 验收产物。

本次本地状态与拼音搜索改动后，`pnpm --filter client test` 通过 32 个文件、165 项测试，`pnpm --filter client build` 通过。下方发布验收是改动前的历史记录；本次没有重新运行其生产容器或真实浏览器专项。
